...
def affine_forward(x, w, b):
    ...
    ###########################################################################
    N = x.shape[0]
    x1 = np.reshape(x, (N,-1), order='C')
    out = np.matmul(x1,w)+b
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, w, b)
...
def affine_backward(dout, cache):
    ...
    D = w.shape[0]
    pass
    dw = np.matmul(np.transpose(x.reshape((N,D))), dout)
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        variance_x = np.var(x-mean_x, axis=0)
        xcorrected = (x-mean_x)
        std = np.sqrt(variance_x+eps)
        istd = 1/std
        xhat = xcorrected*istd
        y = xhat*gamma + beta
        # print 'momentum shape: ' + str(momentum.shape)
        # print 'running mean shape: ' + str(running_mean.shape)
        # print 'mean_x shape: ' + str(mean_x.shape)
        running_mean = momentum*running_mean + (1-momentum)*mean_x
        running_var = momentum*running_var + (1-momentum)*variance_x
        out = y
        ...
        #######################################################################       
        # print "running_var: " + str(running_var+eps)
...
def dropout_forward(x, dropout_param):
    ...
    if mode == 'train':
        ...
        #######################################################################
        # rand_gen = np.random.uniform(0,1,x.shape)
        # mask = (rand_gen<p)
