...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    ##############################################################################
    # https://github.com/yunjey/cs231n/tree/master/assignment3
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    prev_h = h0
    for ii in range(T):        
        prev_h, cache[ii] = rnn_step_forward(x[:,ii,:], prev_h, Wx, Wh, b)
        h[:,ii,:] =  prev_h
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    i0, f0, o0, g0 = np.split(a, 4, axis = 1)
    i = sigmoid(i0)
    f = sigmoid(f0)
    o = sigmoid(o0)
    g = np.tanh(g0)
    ...
    cache = (x, prev_h, prev_c, next_h, Wx, Wh, i, f, o, g, next_c)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    #############################################################################
    x, prev_h, prev_c, next_h, Wx, Wh, i, f, o, g, next_c = cache
...
def lstm_backward(dh, cache):
    ...
    #############################################################################
    N, T, H = dh.shape
    x, prev_h, prev_c, next_h, Wx, Wh, i, f, o, g, next_c = cache[0]
    D = x.shape[1]
    dx, dh0, dWx, dWh, db = np.zeros((N,T,D)), np.zeros((N,H)), np.zeros((D,4*H)), np.zeros((H,4*H)), np.zeros((4*H,))
    dnext_h = 0
    dnext_c = 0
    for t in reversed(range(T)):
        dx[:,t,:], dnext_h, dnext_c, dWx_t, dWh_t, db_t = lstm_step_backward(dnext_h+dh[:,t,:], dnext_c, cache[t])
        dWx += dWx_t
        dWh += dWh_t
        db += db_t
    dh0 = dnext_h
